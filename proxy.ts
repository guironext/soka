...
const ROLE_ACCESS = [
  ...
  { role: "DEPARTMENT_COMITE_NATIONAL" as const, base: "/departement-comite-national" },
  { role: "DEPARTMENT_CENTRE_GENERAL" as const, base: "/departement-centre-general" },
...
const isComiteNationalAppRoute = createRouteMatcher(["/comite_national", "/comite_national/(.*)"]);
const isCentreGeneralAppRoute = createRouteMatcher(["/centre_general", "/centre_general/(.*)"]);
...
const isSousGroupeAppRoute = createRouteMatcher(["/sous_groupe", "/sous_groupe/(.*)"]);
const isMembreAppRoute = createRouteMatcher(["/membre", "/membre/(.*)"]);
const isDepartementComiteNationalAppRoute = createRouteMatcher(["/departement-comite-national", "/departement-comite-national/(.*)"]);
const isDepartementCentreGeneralAppRoute = createRouteMatcher(["/departement-centre-general", "/departement-centre-general/(.*)"]);
const isDepartementCentreAppRoute = createRouteMatcher(["/departement-centre", "/departement-centre/(.*)"]);
const isDepartementChapitreAppRoute = createRouteMatcher(["/departement-chapitre", "/departement-chapitre/(.*)"]);
const isDepartementDistrictAppRoute = createRouteMatcher(["/departement-district", "/departement-district/(.*)"]);
const isDepartementGroupeAppRoute = createRouteMatcher(["/departement-groupe", "/departement-groupe/(.*)"]);
const isDepartementSousGroupeAppRoute = createRouteMatcher(["/departement-sous-groupe", "/departement-sous-groupe/(.*)"]);
const isDepartementMembreAppRoute = createRouteMatcher(["/departement-membre", "/departement-membre/(.*)"]);
/**
...
function sokaRoleFromClaims(
  ...
  if (fromMetaSoka && fromMetaSoka in ROLE_HOME) return fromMetaSoka as SokaRole;
  const fromMetaLegacy = claims.metadata?.role;
  if (fromMetaLegacy && fromMetaLegacy in ROLE_HOME) return fromMetaLegacy as SokaRole;
...
function redirectForPendingTargetIfNeeded(
  ...
  if (!appRole || !pending || !ROLES_WITH_PENDING_TARGET_REDIRECT.has(appRole)) return null;
...
export default clerkMiddleware(
  async (auth, req: NextRequest) => {
    ...
    if (process.env.NODE_ENV !== "production") {
      const mdAny = md as
        | { sokaRole?: unknown; role?: unknown; sokaPendingTargetRole?: unknown }
        | undefined;
      console.log("[mw]", pathname, {
        ...
        rawSokaRoleRoot: (sessionClaims as { sokaRole?: unknown } | undefined)?.sokaRole,
    ...
    if (
      ...
      ROLE_ACCESS.some(({ base }) => pathname.toLowerCase().startsWith(`${base}/`))
    ) {
