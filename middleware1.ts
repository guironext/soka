...
/**
 * Approvals + issuing invitations: must be reachable before Clerk `onboardingCompleted`
 * is true. Authorization is enforced in each route (DB role), not JWT — same idea as
 * {@link isAdminAppRoute} where the UI trusts the database when claims lag.
 */
